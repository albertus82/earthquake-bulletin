...
public class RandomBulletinProvider implements BulletinProvider {
	...
	public Optional<Bulletin> getBulletin(final SearchRequest jobVariables, final BooleanSupplier canceled) {
		...
		for (int i = 0; i < 20; i++) {
			...
			earthquakes.add(new Earthquake(uuid, date.getTime().toInstant().atZone(ZoneId.of("UTC")), magnitude, new Latitude(latitude), new Longitude(longitude), Depth.valueOf(depth), status, uuid, null, null, null));
		}
