...
public enum Preference implements IPreference {
	...
	GEOFON_BASE_URL(new PreferenceDetailsBuilder(ADVANCED).defaultValue(GeofonUtils.DEFAULT_GEOFON_BASE_URL).build(), new FieldEditorDetailsBuilder(ValidatedComboFieldEditor.class).emptyStringAllowed(false).boldCustomValues(true).labelsAndValues(new StaticLabelsAndValues(GeofonUtils.GEOFON_BASE_URL, GeofonUtils.GEOFON_BASE_URL)).build()),
