...
import io.github.albertus82.eqbulletin.service.decode.quakeml.QuakemlBulletinDecoder;
...
@Slf4j
public class QuakemlBulletinDownloader extends ResilientDownloader implements BulletinDownloader {
	...
	static {
		try {
			jaxbContext = JAXBContext.newInstance(Quakeml.class);
		}
		catch (final JAXBException e) {
			throw new IllegalStateException("Cannot create instance of " + JAXBContext.class + " for " + Quakeml.class, e);
		}
	}
	...
	private String fetch(@NonNull final SearchRequest request, final Headers headers, final BooleanSupplier canceled) throws IOException, URISyntaxException, CancelException {
		...
		final StringBuilder uriBuilder = new StringBuilder(request.toURIs().get(0).toString());
